...
# for phrase-2 with unlabel data

def label_captured_data(prob_config: ProblemConfig):
    X_train = pd.read_parquet(prob_config.train_x_path).to_numpy()
    y_train = pd.read_parquet(prob_config.train_y_path).to_numpy()
    ...
    n_samples = len(X_train) + n_captured
    ...
    n_cluster = int(n_samples / 10) * len(np.unique(y_train))
    ...
    for i in range(n_cluster):
        ...
        cluster_labels = X_train[mask]  # labels of data points in cluster i
